...
func _physics_process(delta):
	if GLOBAL.player3life == false and visible == true:
		GLOBAL.palive -= 1
		if GLOBAL.palive == 1:
...
func mouvement_loop():	
	...
	if compy == 0:
		...
		if Input.is_action_pressed("ui_right3") and Input.is_action_pressed("ui_left3"):
			vel.x = 0

		elif Input.is_action_pressed("ui_right3"):
			vel.x = speed	
			compx += 1
			direction = 2
			Fleche.rotation_degrees = -90
			Colision.rotation_degrees = -90
		elif Input.is_action_pressed("ui_left3"):
			vel.x = -speed
			compx += 1
			direction = 4
			Fleche.rotation_degrees = 90
	...
	if compx == 0:
		...
		if Input.is_action_pressed("ui_up3") and Input.is_action_pressed("ui_down3"):
			vel.y = 0
		
		elif Input.is_action_pressed("ui_up3"):
			vel.y = -speed	
			compy +=1
			direction = 1
			Fleche.rotation_degrees = 180
			Colision.rotation_degrees = 180
		elif Input.is_action_pressed("ui_down3"):
			vel.y = speed
			compy += 1
			direction = 3
	...
	if GLOBAL.player3bump > 0:
		if GLOBAL.player3bump != tempbump:
			bumpy = 0
		if GLOBAL.player3bump == 1:
			if bumpy == 0:
				bumpy = -150
			vel.y = bumpy
			bumpy += 5
			if bumpy == -5:
				bumpy = 0
				GLOBAL.player3bump = 0
		if GLOBAL.player3bump == 3:
			if bumpy == 0:
				bumpy = 150
			vel.y = bumpy
			bumpy -= 5
	...
	if GLOBAL.player3life == true:
		if vel.y + vel.x != 0:
			if body.is_in_group("player"):
				if GLOBAL.player3bump > 0 and GLOBAL.lastplayerbump != 1:
					GLOBAL.player1bump = GLOBAL.player3bump
				else:
					if direction == 1:
				...
				GLOBAL.lastplayerbump = 3
					
			if body.is_in_group("player2"):
				if GLOBAL.player3bump > 0 and GLOBAL.lastplayerbump != 2:
					GLOBAL.player2bump = GLOBAL.player3bump
				else:
					if direction == 1:
			...
			if body.is_in_group("player4"):
				if GLOBAL.player3bump > 0 and GLOBAL.lastplayerbump != 4:
					GLOBAL.player4bump = GLOBAL.player3bump
				else:
					if direction == 1:
