...
func _physics_process(delta):
	if GLOBAL.player2life == false and visible == true:
		GLOBAL.palive -= 1
		if GLOBAL.palive == 1:
...
#	z_index = GLOBAL.index2
	
	mouvement_loop()
#	GLOBAL.yplayer2 = position.y
...
func mouvement_loop():	
	...
	if GLOBAL.player2bump > 0:
		if GLOBAL.player2bump != tempbump:
			bumpy = 0
		if GLOBAL.player2bump == 1:
			if bumpy == 0:
				bumpy = -150
			vel.y = bumpy
			bumpy += 5
			if bumpy == -5:
				bumpy = 0
				GLOBAL.player2bump = 0
		if GLOBAL.player2bump == 3:
			if bumpy == 0:
				bumpy = 150
			vel.y = bumpy
			bumpy -= 5
	...
	if GLOBAL.player2life == true:
		if vel.y + vel.x != 0:
			if body.is_in_group("player"):
				if GLOBAL.player2bump > 0 and GLOBAL.lastplayerbump != 1:
					GLOBAL.player1bump = GLOBAL.player2bump
				else:
					if direction == 1:
				...
				GLOBAL.lastplayerbump = 2
			
			if body.is_in_group("player3"):
				if GLOBAL.player2bump > 0 and GLOBAL.lastplayerbump != 3:
					GLOBAL.player3bump = GLOBAL.player2bump
				else:
					if direction == 1:
			...
			if body.is_in_group("player4"):
				if GLOBAL.player2bump > 0 and GLOBAL.lastplayerbump != 4:
					GLOBAL.player4bump = GLOBAL.player2bump
				else:
					if direction == 1:
